...
shinyServer(function(input, output, session) {
  ...
  output$paraPlot <- renderPlot({
    ...
    plot(parasiteDensity_2D()[,2], xlab="Time (hours)", ylab="Parasite density (log10)",xlim=c(0,245),ylim=c(0,10), type = 'l')
    text(x = 120, y=7, paste("Sum of observable parasites: ", sum(round(parasiteDensity_2D()[,2])),"\n Min parasite: ", min(parasiteDensity_2D()[,3])))
  })
  ...
  output$drugeffPlot <- renderPlot({
    ...
    plot(drugEff_A(), xlim=c(0,245),ylim=c(0,max(c(max(drugEff_A()[,2]),max(drugEff_B()[,2])))), type = 'l', ylab="Drug effect (1000*log10)", col="blue")
    par(new=TRUE)
    plot(drugEff_B(), axes=FALSE, xlab="", ylab="", xlim=c(0,245),ylim=c(0,max(c(max(drugEff_A()[,2]),max(drugEff_B()[,2])))), type = 'l',  col="red")
  })
