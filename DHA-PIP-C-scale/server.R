...
shinyServer(function(input, output, session) {
  ...
  ce50_3_R <- reactive(10^input$ce50_3)
  ...
  output$paraPlot_DHApip <- renderPlot({
    ...
    parasiteDensity_DHApip <- reactive(NJWIm_DHApip_C_scale(initn_R(),48,input$mu,input$sig,input$pmf,k0,a,tpar,delay,2400,
                                           killrate_R(),ce50_R(),input$h, 
                                           killrate_2_R(),ce50_2_R(), input$h_2,
                                           killrate_3_R(),ce50_3_R(), input$h_3,input$C_scale)) #input$C_scale
    #output
  ...
  output$DHA_PIP_Plot <- renderPlot({
    ...
    par(new=TRUE)
    plot(drugConc_C()[,1],drugConc_C()[,2], axes=FALSE,xlab="", ylab="",xlim=c(0,400),log="y",ylim=c(10^0, 10^3.5), type = 'l', col="black")
    eaxis(2, n.axp=1) #pretty10exp(10^(0:3),drop.1 = TRUE)
    eaxis(1)
  ...
  output$drugeffPlot_DHApip <- renderPlot({
    ...
    drugEff_C <- reactive(SpecificDrugEffect(2400, input$C_scale,killrate_3_R(),ce50_3_R(),input$h_3))
